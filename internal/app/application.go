...
const mutationLockHeldMessage = "another local-agent process is using the database; wait for it to finish"

var schemaBehindMessage = fmt.Sprintf("database schema is behind this binary's v%d; run local-agent db upgrade first", rollout.TargetVersion)
...
// texts; every other error keeps its own shape. A schema inside [33, 44]
// keeps the upgrade-first message because db upgrade accepts it; a schema
// outside [33, 45] maps to the terminal message that never recommends
