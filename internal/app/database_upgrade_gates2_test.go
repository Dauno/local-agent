...
func corruptFixtureCases() []struct {
	...
	return []struct {
		...
		{"baseline-without-cutoff", 45, map[string]string{keyBaselineStr: "jobs=0;activations=0"}, keyBaselineStr},
		{"cutoff-without-baseline", 45, map[string]string{keyCutoffStr: "12345"}, keyCutoffStr},
		{"malformed-baseline", 45, map[string]string{keyBaselineStr: "bogus"}, keyBaselineStr},
		{"malformed-cutoff", 45, map[string]string{keyCutoffStr: "soon"}, keyCutoffStr},
		{"partial-backup-identity", 45, partialIdentity, keyBackupVerified},
		{"marker-beside-identity", 45, appendSeed(rowTwoBase, keyNotRequired, validTime), keyNotRequired},
		{"relative-backup-path", 45, merge(rowTwoBase, map[string]string{keyBackupPath: "relative.db"}), keyBackupPath},
		{"malformed-bytes", 45, merge(rowTwoBase, map[string]string{keyBackupBytes: "big"}), keyBackupBytes},
		{"short-sha", 45, merge(rowTwoBase, map[string]string{keyBackupSHA: "abc"}), keyBackupSHA},
		{"source-version-zero", 45, merge(rowTwoBase, map[string]string{keyBackupSource: "0"}), keyBackupSource},
		{"bad-verified-at", 45, merge(rowTwoBase, map[string]string{keyBackupVerified: "not-a-time"}), keyBackupVerified},
		{"bad-not-required-at", 45, map[string]string{keyNotRequired: "not-a-time"}, keyNotRequired},
		{"unknown-postflight", 45, map[string]string{keyPostStatus: "weird"}, keyPostStatus},
		{"status-without-detail", 45, map[string]string{keyPostStatus: "passed"}, keyPostStatus},
		{"detail-without-status", 45, map[string]string{keyPostDetail: "orphan detail"}, keyPostDetail},
		{"identity-without-baseline", 45, map[string]string{
	...
	for _, version := range []int{20, 14, 0} {
		t.Run("v"+strconv.Itoa(version), func(t *testing.T) {
			...
			if !errors.As(previewErr, &unsupported) || unsupported.Found != version ||
				...
				t.Fatalf("preview err = %v (%T), want UnsupportedSourceSchemaError{%d,[33,44]}", previewErr, previewErr, version)
			}
