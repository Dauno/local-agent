...
func TestApplyRowOneFullRunPinsFrozenOrdering(t *testing.T) {
	...
	if !report.RolloutAdvanced || report.ToVersion != rollout.TargetVersion || !report.PostflightOK {
		t.Fatalf("report = %+v", report)
	}
	if got := queryUserVersion(t, dbPath); got != rollout.TargetVersion {
		t.Fatalf("user_version = %d, want %d", got, rollout.TargetVersion)
	}
...
func TestAdoptionWritesFixedZeroWithoutMeasuringOrMigrating(t *testing.T) {
	...
	if got := queryUserVersion(t, h.paths.DatabaseFile); got != rollout.TargetVersion {
