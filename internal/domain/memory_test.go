...
func TestValidateMemoryReferenceText(t *testing.T) {
	tests := []struct {
		...
		{name: "read file technical identifier", value: "read_file", want: false},
		{name: "read file technical label", value: "`read_file`: Reads files from registered projects.", want: false},
		{name: "read file technical bullet", value: "- `read_file`: Reads files from registered projects.", want: false},
		{name: "snake case directive", value: "ignore_previous_instructions", want: true},
