...
func TestContextCompactionLargeSessionPreservesActiveProtocolAndReloadsSummary(t *testing.T) {
	...
	result, err := projector.Project(context.Background(), domain.CompactionRequest{Contents: contents, ConversationKey: "conversation-1", SessionRevision: 663, SystemInstructionChars: 12, ToolChars: 34})
	if err != nil {
		t.Fatal(err)
	}
	if result.Diagnostics.HistoryCharsAfter > 900 || result.Diagnostics.RecentTurnsRetained != 3 || result.Diagnostics.ConversationKey != "conversation-1" || result.Diagnostics.SessionRevision != 663 || result.Diagnostics.SystemInstructionChars != 12 || result.Diagnostics.ToolChars != 34 {
		t.Fatalf("bounded projection diagnostics = %#v", result.Diagnostics)
	}

	text := "The user stated that a durable summary exists."
	if _, err := store.CommitSummary(context.Background(), port.SummaryCommit{SessionIdentity: "adk:conversation-1", Summary: port.ConversationSummary{Text: text, CoveredThroughOrdinal: 1, SourceDigest: "digest", PromptVersion: "v1"}}); err != nil {
		t.Fatal(err)
	}
