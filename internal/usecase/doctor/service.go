...
func (s *Service) Run(ctx context.Context, includeLive bool) Report {
	...
	keys := []string{cfg.Model.APIKeyEnv, SlackBotTokenKey, SlackAppTokenKey}
	values, err := s.deps.Secrets.Resolve(keys...)
	if err != nil {
		report.fail("secrets", err.Error(), "Fix .env syntax or process environment values.", false)
		return report
	}
	redactor := secure.NewRedactor(values[cfg.Model.APIKeyEnv], values[SlackBotTokenKey], values[SlackAppTokenKey])
	...
	checkSecret("model API key", cfg.Model.APIKeyEnv, "", "Set "+cfg.Model.APIKeyEnv+" in the process environment or .env.")
	...
	} else if err := s.deps.Database.CheckDatabase(ctx, paths.DatabaseFile); err != nil {
