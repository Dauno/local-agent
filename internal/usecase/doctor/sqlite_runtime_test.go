...
func healthySQLiteRuntime() domain.SQLiteRuntimeHealth {
	return domain.SQLiteRuntimeHealth{
		SchemaVersion: 42, JournalMode: "wal", Synchronous: 2,
...
func TestDoctorReportsHealthySQLiteRuntime(t *testing.T) {
	...
	for _, want := range []string{"schema_version=42", "journal_mode=wal", "synchronous=2", "busy_timeout_ms=5000", "foreign_keys=true", "max_open_connections=4"} {
